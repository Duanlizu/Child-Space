...
- (void)drawRect:(CGRect)rect {
	...
	radius = radius - 2;
	float amo = self.progress * rect.size.width;
	
	// Progress in the middle area
	if (amo >= radius + 4 && amo <= (rect.size.width - radius - 4)) {
		CGContextMoveToPoint(context, 4, rect.size.height/2);
		CGContextAddArcToPoint(context, 4, 4, radius + 4, 4, radius);
		CGContextAddLineToPoint(context, amo, 4);
		CGContextAddLineToPoint(context, amo, radius + 4);
		
		CGContextMoveToPoint(context, 4, rect.size.height/2);
		CGContextAddArcToPoint(context, 4, rect.size.height - 4, radius + 4, rect.size.height - 4, radius);
		CGContextAddLineToPoint(context, amo, rect.size.height - 4);
		CGContextAddLineToPoint(context, amo, radius + 4);
	...
	// Progress in the right arc
	else if (amo > radius + 4) {
		float x = amo - (rect.size.width - radius - 4);
		...
		CGContextAddLineToPoint(context, amo, rect.size.height/2);
